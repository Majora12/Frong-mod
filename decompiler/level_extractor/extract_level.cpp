...
void extract_art_groups_from_level(const ObjectFileDB& db,
  ...
  for (const auto& file : files) {
    if (file.name.length() > 3 && !file.name.compare(file.name.length() - 3, 3, "-ag")) {
      ...
      // extract_joint_group(ag_file, db.dts, db.version(), art_group_data); // Commented out for fodder spawn
    }
  }
}
...
void extract_common(const ObjectFileDB& db,
                    ...
                    const Config& config,
                    const std::string& ruins_dgo = "RUI.DGO") {
  ...
  if (config.make_merc_models_common) {
    // stuff merc models from all levels into GAME.fr3
      const std::string& lvl_dgo_name = ruins_dgo ;
